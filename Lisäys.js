...
const Form = ({addItem, jsonData, handleClick}) => {
  ...
  //varmistaa että ei anna samaa nimeä ja kirjoittajaa eri kirjoille 
    if (jsonData.some(jsonData => jsonData.nimi === newName && jsonData.kirjoittaja === newAuthor)) {
        ...
        <form id="kysely">
          ...
          /><br/>
           <p>Id: {newId}</p><br/>
             ...
             <button id="cancel-button" type="reset" onClick={handleClick}>Peru</button>
        </form>
      </div>
    </div>
  )
